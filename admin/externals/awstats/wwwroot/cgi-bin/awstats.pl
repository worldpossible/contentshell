	...
	foreach my $lng ( split( /\s+/, $ShowFlagLinks ) ) {
		...
		if ( $lng ne $CurrentLang ) {
			...
			print "<a href=\""
			  ...
			  . "\"$NewLinkTarget><img src=\"$DirIcons\/other\/blank.png\" height=\"14\" border=\"0\""
			  . AltTitle("$lngtitle")
			  . " /></a>&nbsp;\n";
		}
	}
...
"<tr bgcolor=\"#F6F6F6\"><td class=\"aws\" colspan=\"2\"><b>$family_name</b></td>";
				...
				print "<td"
				  . ( $count ? "" : " width=\"$WIDTHCOLICON\"" )
				  . "><img src=\"$DirIcons\/os\/$family.png\""
				  . AltTitle("")
				  . " /></td>";
...
"<th class=\"datasize\" bgcolor=\"#$color_k\" width=\"80\">$Message[75]</th>";
	...
	foreach my $key (@keylist) {
		...
		if ( $newkey eq 'ip' || !$DomainsHashIDLib{$newkey} ) {
			print
"<tr><td width=\"$WIDTHCOLICON\"><img src=\"$DirIcons\/other\/blank.png\" height=\"14\""
			  . AltTitle("$Message[0]")
...
"<th bgcolor=\"#$color_h\" width=\"80\">$Message[57]</th><th bgcolor=\"#$color_h\" width=\"80\">$Message[15]</th></tr>\n";
	...
	foreach my $key (@keylist) {
		...
		if ( $key eq 'Unknown' ) {
			...
			my $libos = $OSHashLib{$keywithoutcumul}
			  || $keywithoutcumul;
			my $nameicon = $OSFamily{$keywithoutcumul} ? $keywithoutcumul : "unknown";
			if ( $OSFamily{$keywithoutcumul} ) {
...
print "<noframes><body>Your browser does not support frames.<br>\n";
print "Please try a different browser.<br>\n";
print "</body></noframes>\n";
print "</frameset>\n";
